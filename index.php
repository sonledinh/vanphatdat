<?php include 'header.php';?>
<main>
	<section class="box-banenr">
		<div class="slide-banner">
			<div class="item">
				<div class="avarta"><img src="images/banner.jpg" class="img-fluid w-100" alt=""></div>
				<div class="caption">
					<div class="container">
						<div class="row">
							<div class="col-md-7"></div>
							<div class="col-md-5">
								<div class="info">
									<h4><img src="images/title-cap.png" class="img-fluid" alt=""></h4>
									<h3>Chuyên cung cấp dịch vụ</h3>
									<h2>IT Helpdesk</h2>
								</div>
							</div>
						</div>
					</div>
				</div>
			</div>
			<div class="item">
				<div class="avarta"><img src="images/banner.jpg" class="img-fluid w-100" alt=""></div>
				<div class="caption">
					<div class="container">
						<div class="row">
							<div class="col-md-7"></div>
							<div class="col-md-5">
								<div class="info">
									<h4><img src="images/title-cap.png" class="img-fluid" alt=""></h4>
									<h3>Chuyên cung cấp dịch vụ</h3>
									<h2>IT Helpdesk</h2>
								</div>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div> 
	</section>
	<section class="box-partner">
		<div class="container">
			<div class="title text-center">
				<h2 class="text-uppercase">ĐỐI TÁC VÀ KHÁCH HÀNG</h2>
			</div>
			<div class="desc-part text-center">
				<p>Dựa trên công nghệ mới được phát triển bởi các Công ty Doanh nghiệp lớn như Cisco, IBM, HP, Juniper, Fortigate, Palo Alto… Công ty chúng tôi đã trở thành đối tác của công ty họ tại Việt Nam. Chúng tôi thiết kế cơ sở hạ tầng mạng, tư vấn và cung cấp các giải pháp tốt nhất cho tường lửa, bộ nhớ, hội nghị truyền hình, thoại và dữ liệu CNTT của khách hàng</p>
			</div>
			<div class="list-partner">
				<ul>
					<li><a href=""><img src="images/part-1.png" class="img-fluid" alt=""></a></li>
					<li><a href=""><img src="images/part-2.png" class="img-fluid" alt=""></a></li>
					<li><a href=""><img src="images/part-3.png" class="img-fluid" alt=""></a></li>
					<li><a href=""><img src="images/part-4.png" class="img-fluid" alt=""></a></li>
					<li><a href=""><img src="images/part-5.png" class="img-fluid" alt=""></a></li>
					<li><a href=""><img src="images/part-1.png" class="img-fluid" alt=""></a></li>
					<li><a href=""><img src="images/part-2.png" class="img-fluid" alt=""></a></li>
					<li><a href=""><img src="images/part-3.png" class="img-fluid" alt=""></a></li>
					<li><a href=""><img src="images/part-4.png" class="img-fluid" alt=""></a></li>
					<li><a href=""><img src="images/part-5.png" class="img-fluid" alt=""></a></li>
				</ul>
			</div>
		</div>
	</section>
	<section class="box-about">
		<div class="container">
			<div class="row">
				<div class="col-md-6">
					<div class="about-left">
						<h2>IT HELPDESK là gì ?</h2>
						<div class="desc">
							<p>Chúng tôi triển khai giải pháp công nghệ GPON mới phục vụ nhu cầu của Nhà cung cấp dịch vụ Internet, xây dựng giải pháp dựa trên giải pháp Dasan, ZTE. Service Desk Base in Customer Site. Maintenance & Support during 24/7 for Customer.</p>
						</div>
						<div class="btn-main"><a href="" class="text-uppercase">đăng ký ngay</a></div>
					</div>
				</div>
				<div class="col-md-6">
					<div class="avarta-ab text-right"><img src="images/about.png" class="img-fluid" alt=""></div>
				</div>
			</div>
		</div>
	</section>
	<section class="box-service">
		<div class="container">
			<div class="title text-center">
				<h2 class="text-uppercase">Gói dịch vụ it chuyên nghiệp</h2>
				<div class="desc">
					<p>Dịch vụ xử lý vấn để On-site và On-Call 27/7</p>
				</div>
			</div>
			<div class="list-srv">
				<div class="row">
					<div class="col-md-4">
						<div class="item-srv text-center">
							<div class="icon"><img src="images/srv-1.png" class="img-fluid" alt=""></div>
							<div class="info">
								<h3>DỊCH VỤ IT HELPDESK</h3>
								<div class="desc">Xác định, chẩn đoán, và giải quyết vấn đề ở mức 1 đối với yêu cầu của người sử dụng máy tính cá nhân, phần mềm máy tính và phần cứng, mạng, Internet.</div>
							</div>
						</div>
					</div>
					<div class="col-md-4">
						<div class="item-srv text-center">
							<div class="icon"><img src="images/srv-2.png" class="img-fluid" alt=""></div>
							<div class="info">
								<h3>DỊCH VỤ IT ON-SITE</h3>
								<div class="desc">Xác định, chẩn đoán, và giải quyết vấn đề ở mức 1 đối với yêu cầu của người sử dụng máy tính cá nhân, phần mềm máy tính và phần cứng, mạng, Internet.</div>
							</div>
						</div>
					</div>
					<div class="col-md-4">
						<div class="item-srv text-center">
							<div class="icon"><img src="images/srv-3.png" class="img-fluid" alt=""></div>
							<div class="info">
								<h3>DỊCH VỤ IT HỖ TRỢ TỪ XA</h3>
								<div class="desc">Xác định, chẩn đoán, và giải quyết vấn đề ở mức 1 đối với yêu cầu của người sử dụng máy tính cá nhân, phần mềm máy tính và phần cứng, mạng, Internet.</div>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</section>
	<section class="box-quesstion">
		<div class="container">
			<div class="title text-center">
				<h2 class="text-uppercase">Các câu hỏi thường gặp từ khách hàng</h2>
				<div class="desc">
					<p>Dịch vụ xử lý vấn để On-site và On-Call 27/7</p>
				</div>
			</div>
			<div class="list-quess">
				<div class="item-quess">
					<div class="head-quess">
						<h4>Từ khi nhận đợc yêu cầu đến khi support là bao lâu ?</h4>
					</div>
					<div class="answer">
						<p>Xác định, chẩn đoán, và giải quyết vấn đề ở mức 1 đối với yêu cầu của người sử dụng máy tính cá nhân, phần mềm máy tính và phần cứng, mạng, Internet.Cung cấp hỗ trợ cài đặt, cấu hình cho người dùng cuối các phần cứng máy tính để bàn máy tính, phần mềm và thiết bị ngoại vi.</p>
						<p>Sửa chữa đúng hạn các thiết bị máy tính PC hoặc bảo hành, bảo trì từ nhà cung cấp bên thứ ba theo thỏa thuận hợp đồng.</p>
						<p>Thực hiện sửa chữa phần cứng máy tính, các thiết bị máy tính PC và các thiết bị ngoại vi không được thỏa thuận bảo dưỡng của bên thứ ba nhà cung cấp.</p>
					</div>
				</div>
				<div class="item-quess">
					<div class="head-quess">
						<h4>Từ khi nhận đợc yêu cầu đến khi support là bao lâu ?</h4>
					</div>
					<div class="answer">
						<p>Xác định, chẩn đoán, và giải quyết vấn đề ở mức 1 đối với yêu cầu của người sử dụng máy tính cá nhân, phần mềm máy tính và phần cứng, mạng, Internet.Cung cấp hỗ trợ cài đặt, cấu hình cho người dùng cuối các phần cứng máy tính để bàn máy tính, phần mềm và thiết bị ngoại vi.</p>
						<p>Sửa chữa đúng hạn các thiết bị máy tính PC hoặc bảo hành, bảo trì từ nhà cung cấp bên thứ ba theo thỏa thuận hợp đồng.</p>
						<p>Thực hiện sửa chữa phần cứng máy tính, các thiết bị máy tính PC và các thiết bị ngoại vi không được thỏa thuận bảo dưỡng của bên thứ ba nhà cung cấp.</p>
					</div>
				</div>
				<div class="item-quess">
					<div class="head-quess">
						<h4>Từ khi nhận đợc yêu cầu đến khi support là bao lâu ?</h4>
					</div>
					<div class="answer">
						<p>Xác định, chẩn đoán, và giải quyết vấn đề ở mức 1 đối với yêu cầu của người sử dụng máy tính cá nhân, phần mềm máy tính và phần cứng, mạng, Internet.Cung cấp hỗ trợ cài đặt, cấu hình cho người dùng cuối các phần cứng máy tính để bàn máy tính, phần mềm và thiết bị ngoại vi.</p>
						<p>Sửa chữa đúng hạn các thiết bị máy tính PC hoặc bảo hành, bảo trì từ nhà cung cấp bên thứ ba theo thỏa thuận hợp đồng.</p>
						<p>Thực hiện sửa chữa phần cứng máy tính, các thiết bị máy tính PC và các thiết bị ngoại vi không được thỏa thuận bảo dưỡng của bên thứ ba nhà cung cấp.</p>
					</div>
				</div>
			</div>
			<div class="load-more text-center">
				<a href="">Xem thêm</a>
			</div>
		</div>
	</section>
	<section class="box-benefit" style="background: url('images/bn-ben.jpg') no-repeat center;">
		<div class="container">
			<div class="title text-center">
				<h2 class="text-uppercase">Lợi ích của việc thuê dịch vụ cntt vạn phát đạt</h2>
				<div class="desc">
					<p>Dịch vụ xử lý vấn để On-site và On-Call 27/7</p>
				</div>
			</div>
			<div class="list-benefit">
				<div class="row">
					<div class="col-md-4">
						<div class="item-benefit text-center">
							<div class="avarta"><img src="images/l-ich-1.jpg" class="img-fluid w-100" alt=""></div>
							<div class="info">
								<h4>Lợi ích về nguồn nhân lực</h4>
								<div class="desc">
									<p>Tập trung nguồn lực: Các tổ chức sau khi thuê ngoài dịch vụ CNTT sẽ có thêm thời gian và nguồn lực để tập trung vào những nhiệm vụ then chốt và cốt lõi. Nhà quản trị CNTT của tổ chức sẽ có thời gian để tập trung vào việc phát triển các định hướng chiến lược về CNTT đáp ứng tốt hơn chiến lược nghiệp vụ của tổ chức và tập trung vào việc quản trị quy trình thuê ngoài dịch vụ CNTT để đạt hiệu quả cao nhất.</p>
									<p>Hiệu suất: Nhà cung cấp dịch vụ ITO có thể điều phối nhân lực, do đó với những nhân lực quý hiếm, </p>
								</div>
							</div>
						</div>
					</div>
					<div class="col-md-4">
						<div class="item-benefit text-center">
							<div class="avarta"><img src="images/l-ich-2.jpg" class="img-fluid w-100" alt=""></div>
							<div class="info">
								<h4>Lợi ích về nguồn nhân lực</h4>
								<div class="desc">
									<p>Tập trung nguồn lực: Các tổ chức sau khi thuê ngoài dịch vụ CNTT sẽ có thêm thời gian và nguồn lực để tập trung vào những nhiệm vụ then chốt và cốt lõi. Nhà quản trị CNTT của tổ chức sẽ có thời gian để tập trung vào việc phát triển các định hướng chiến lược về CNTT đáp ứng tốt hơn chiến lược nghiệp vụ của tổ chức và tập trung vào việc quản trị quy trình thuê ngoài dịch vụ CNTT để đạt hiệu quả cao nhất.</p>
									<p>Hiệu suất: Nhà cung cấp dịch vụ ITO có thể điều phối nhân lực, do đó với những nhân lực quý hiếm, </p>
								</div>
							</div>
						</div>
					</div>
					<div class="col-md-4">
						<div class="item-benefit text-center">
							<div class="avarta"><img src="images/l-ich-3.jpg" class="img-fluid w-100" alt=""></div>
							<div class="info">
								<h4>Lợi ích về nguồn nhân lực</h4>
								<div class="desc">
									<p>Tập trung nguồn lực: Các tổ chức sau khi thuê ngoài dịch vụ CNTT sẽ có thêm thời gian và nguồn lực để tập trung vào những nhiệm vụ then chốt và cốt lõi. Nhà quản trị CNTT của tổ chức sẽ có thời gian để tập trung vào việc phát triển các định hướng chiến lược về CNTT đáp ứng tốt hơn chiến lược nghiệp vụ của tổ chức và tập trung vào việc quản trị quy trình thuê ngoài dịch vụ CNTT để đạt hiệu quả cao nhất.</p>
									<p>Hiệu suất: Nhà cung cấp dịch vụ ITO có thể điều phối nhân lực, do đó với những nhân lực quý hiếm, </p>
								</div>
							</div>
						</div>
					</div>
				</div>
				<div class="view-all text-center"><a href="">XEM TẤT CẢ</a></div>
			</div>
		</div>
	</section>
	<section class="box-news">
		<div class="container">
			<div class="title text-center">
				<h2 class="text-uppercase">tin tức</h2>
				<div class="desc">
					<p>Dịch vụ xử lý vấn để On-site và On-Call 27/7</p>
				</div>
			</div>
			<div class="list-news">
				<div class="row">
					<div class="col-md-4">
						<div class="item-news">
							<div class="avarta"><img src="images/new-1.png" class="img-fluid w-100" alt=""></div>
							<div class="info">
								<h4><a href="">Ưu điểm và nhược điểm của máy chủ cho thuê</a></h4>
								<div class="desc">
									Để sử dụng máy chủ, nếu phân chia ra sẽ có hai hướng lựa chọn lớn, đó là “tự mình mua” hoặc “đi thuê”. Dưới đây là ưu nhược điểm của dịch vụ cho thuê máy chủ, Việc quản lý máy chủ sẽ khác nhau...
								</div>
							</div>
							<div class="info-abs" style="background: url('images/new-1.jpg') no-repeat center;">
								<div class="i-abs text-center">
									<h3>Server là gì? Máy chủ là gì? Những điều cần biết về Server.</h3>
									<div class="desc-abs">Server hay còn gọi là máy chủ là một hệ thống (phần mềm và phần cứng máy tính phù hợp) đáp ứng yêu cầu trên một mạng máy tính để cung cấp, hoặc hỗ trợ cung cấp một dịch vụ mạng. Các server có thể chạy trên một </div>
								</div>
							</div>
						</div>
					</div>
					<div class="col-md-4">
						<div class="item-news">
							<div class="avarta"><img src="images/new-2.png" class="img-fluid w-100" alt=""></div>
							<div class="info">
								<h4><a href="">Ưu điểm và nhược điểm của máy chủ cho thuê</a></h4>
								<div class="desc">
									Để sử dụng máy chủ, nếu phân chia ra sẽ có hai hướng lựa chọn lớn, đó là “tự mình mua” hoặc “đi thuê”. Dưới đây là ưu nhược điểm của dịch vụ cho thuê máy chủ, Việc quản lý máy chủ sẽ khác nhau...
								</div>
							</div>
							<div class="info-abs" style="background: url('images/new-1.jpg') no-repeat center;">
								<div class="i-abs text-center">
									<h3>Server là gì? Máy chủ là gì? Những điều cần biết về Server.</h3>
									<div class="desc-abs">Server hay còn gọi là máy chủ là một hệ thống (phần mềm và phần cứng máy tính phù hợp) đáp ứng yêu cầu trên một mạng máy tính để cung cấp, hoặc hỗ trợ cung cấp một dịch vụ mạng. Các server có thể chạy trên một </div>
								</div>
							</div>
						</div>
					</div>
					<div class="col-md-4">
						<div class="item-news">
							<div class="avarta"><img src="images/new-1.png" class="img-fluid w-100" alt=""></div>
							<div class="info">
								<h4><a href="">Ưu điểm và nhược điểm của máy chủ cho thuê</a></h4>
								<div class="desc">
									Để sử dụng máy chủ, nếu phân chia ra sẽ có hai hướng lựa chọn lớn, đó là “tự mình mua” hoặc “đi thuê”. Dưới đây là ưu nhược điểm của dịch vụ cho thuê máy chủ, Việc quản lý máy chủ sẽ khác nhau...
								</div>
							</div>
							<div class="info-abs" style="background: url('images/new-1.jpg') no-repeat center;">
								<div class="i-abs text-center">
									<h3>Server là gì? Máy chủ là gì? Những điều cần biết về Server.</h3>
									<div class="desc-abs">Server hay còn gọi là máy chủ là một hệ thống (phần mềm và phần cứng máy tính phù hợp) đáp ứng yêu cầu trên một mạng máy tính để cung cấp, hoặc hỗ trợ cung cấp một dịch vụ mạng. Các server có thể chạy trên một </div>
								</div>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</section>
</main>
<?php include 'footer.php';?>     
      
      